...
namespace ServerProfile
{
    public partial class POSManager
    {
        ...
        private void ParseReportFormToXml(XmlDocument xmlDoc, XmlNode formNode, ReportForm reportForm, IServer server)
        {
            ...
            var registerId = new List<UInt16>(reportForm.POS);
            registerId.Sort();
            ...
            String body = String.Join(",", exceptions.ToArray()) + Environment.NewLine;

            if (registerId.Count > 0)
                body += "(POS " + posIds + ")" + Environment.NewLine;
        ...
        private void ParseReportFormFromXml(ReportForm reportForm, XmlNode formNode)
        {
            ...
            //}
            var registerId = Xml.GetFirstElementValueByTagName(formNode, "POS").Split(',').Where(id => !String.IsNullOrEmpty(id)).Select(id => Convert.ToUInt16(id));
            ...
            foreach (XmlNode requestNode in requestNodeList)
            {
                ...
                if (registerIdsNode != null)
                {
                    foreach (XmlElement childNode in registerIdsNode.ChildNodes)
                    {
                        ...
                        {
                            POSId = Convert.ToUInt16(childNode.InnerText),
