...
namespace App
{
    public partial class LoginForm : Form
    {
        ...
        {
            //check is user(windows user) have administrator permission to run system lock function
            AppClient.IsAdministrator = false;
            try
            {
                ...
                if (user != null)
                {
                    ...
                    AppClient.IsAdministrator = principal.IsInRole(WindowsBuiltInRole.Administrator);
                }
            }
            ...
            // Disable Security System on start up(if AP lock and crash)
            var appClient = new AppClient();
            appClient.ReleaseKeyboardHook();
            appClient.ShowToolbar();
            appClient.EnableTaskManager();

            ServerCredential credential = null;

            //auto login via pass argument from call NVR.exe / or from "auto login in" setting
            if (arguments != null && arguments.Length > 0)
            {
                credential = new ServerCredential();

                if (arguments.Length > 1)
                    credential.Domain = arguments[0];

                if (arguments.Length > 2)
                    credential.Port = Convert.ToUInt16(arguments[1]);

                if (arguments.Length > 3)
                    credential.UserName = arguments[2];

                if (arguments.Length > 4)
                    credential.Password = arguments[3];

                if (arguments.Length > 5)
                    credential.SSLEnable = (arguments[4] == "true");

                if (arguments.Length > 6)
                    AppProperties.DefaultLanguage = arguments[5];

                if (arguments.Length > 7)
                    _focusPage = arguments[6];
            ...
            //auto login via saved property, normal it's because exe crash
            if (credential == null && File.Exists(file))
            {
                ...
                if (code != "")
                {
                    ...
                    try
                    {
                        ...
                        if (!nvrStillAlive)
                        {
                            var account = Xml.GetFirstElementValueByTagName(xmlDoc, "Account");
                            if (!String.IsNullOrEmpty(account))
                            {
                                credential = new ServerCredential
                                {
                                    Domain = Xml.GetFirstElementValueByTagName(xmlDoc, "Doamin"),
                                    Port = Convert.ToUInt16(Xml.GetFirstElementValueByTagName(xmlDoc, "Port")),
                                    UserName = account,
                                    Password = Xml.GetFirstElementValueByTagName(xmlDoc, "Password"),
            ...
            if (credential == null && AppProperties.DefaultAutoSignIn)
            {
                var values = ServerProperties.LoadCredentialHistory(AppProperties.DefaultHistory).Values;
                if (values.Count > 0)
                {
                    String temp = values.First();
                    credential = ServerProperties.ParseDesStringToCredential(temp);
                }
            }

            if (credential != null && AutoLogin(credential))
                return true;
            // no auto-login before form shown, load the form for user
