...
namespace SetupException
{
    public sealed class TagPanel : Panel
    {
        ...
        public new POS_Exception.Tag Tag
        {
            set
            {
                _tag = value;
        ...
        {
            if (Width <= 330) return;
            Manager.PaintTitleText(g, Localization["Tag_Tag"]);

            if (Width <= 450) return;
            g.DrawString(Localization["Tag_Value"], Manager.Font, Manager.TitleTextColor, Width - 196, 13);
        }

        private void TagPanelPaint(Object sender, PaintEventArgs e)
        {
