...
namespace DeviceCab
{
    public class InskytecCameraModel : CameraModel
    {
        public List<TvStandard> TvStandard = new List<TvStandard>();

        public List<InskytecProfileSetting> ProfileSettings = new List<InskytecProfileSetting>();

        private static List<InskytecProfileSetting> FindProfileSettingByCondition(IEnumerable<InskytecProfileSetting> nodes , StreamConfig condition)
        {
            var profileSettings = new List<InskytecProfileSetting>();

            foreach (InskytecProfileSetting node in nodes)
            {
                foreach (InskytecCompression compression in node.Compressions)
                {
                    if (compression.Compression == condition.Compression)
                    {
                        foreach (InskytecResolution resolution in compression.Resolutions)
                        {
                            if (resolution.Resolutions.IndexOf(condition.Resolution) > -1)
                            {
                                foreach (InskytecFrameRate frameRate in resolution.FrameRates)
                                {
        ...
        public Compression[] GetCompressionByCondition(UInt16 profileId, TvStandard tvStandard, CameraMode mode, ConnectionProtocol connectionProtocol, StreamConfig condition1, StreamConfig condition2, StreamConfig condition3)
        {
            ...
            InskytecProfileSetting targetProfile1 = null;
            ...
            //find to profile2
            var targetProfile2 = FindProfileSettingByCondition(new List<InskytecProfileSetting>{targetProfile1}, condition1);
        ...
        public Compression[] GetCompressionsByProfileSetting(IEnumerable<InskytecCompression> profileCompressions, ConnectionProtocol connectionProtocol)
        {
            var compressions = new List<Compression>();

            foreach (InskytecCompression compression in profileCompressions)
            {
        ...
        public UInt16[] GetFramerateByCondition(UInt16 profileId, TvStandard tvStandard, CameraMode mode, StreamConfig condition1, StreamConfig condition2, StreamConfig condition3, StreamConfig condition4)
        {
            ...
            //find to profile1
            if (targetProfile1 == null) return null;
            //if (condition2 == null) return GetFrameRateByProfileSetting(new List<InskytecProfileSetting>{targetProfile1}, condition1);
            return GetFrameRateByProfileSetting(new List<InskytecProfileSetting> { targetProfile1 }, condition1);
            //find to profile2
            var targetProfile2 = FindProfileSettingByCondition(new List<InskytecProfileSetting>{targetProfile1}, condition1);
            if (targetProfile2 == null) return null;
        ...
        public UInt16[] GetFrameRateByProfileSetting(List<InskytecProfileSetting>profileSettings, StreamConfig condiction)
        {
            var list = new List<UInt16[]>();

            foreach (InskytecProfileSetting profileSetting in profileSettings)
            {
                foreach (InskytecCompression compression in profileSetting.Compressions)
                {
        ...
        public Bitrate[] GetBiterateByProfileSetting(InskytecProfileSetting profileSetting, StreamConfig condiction)
        {
            foreach (InskytecCompression compression in profileSetting.Compressions)
            {
                if (compression.Compression == condiction.Compression)
        ...
        public Resolution[] GetResolutionByCondition(UInt16 profileId, TvStandard tvStandard, CameraMode mode, StreamConfig condition1, StreamConfig condition2, StreamConfig condition3, StreamConfig condition4)
        {
            ...
            //find to profile1
            if (targetProfile1 == null) return null;
            //if (condition2 == null) return GetResolutionByProfileSetting(new List<InskytecProfileSetting>{ targetProfile1 }, condition1);
            return GetResolutionByProfileSetting(new List<InskytecProfileSetting> { targetProfile1 }, condition1);
            //find to profile2
            var targetProfile2 = FindProfileSettingByCondition(new List<InskytecProfileSetting>{targetProfile1}, condition1);
            if (targetProfile2 == null) return null;
        ...
        public Resolution[] GetResolutionByProfileSetting(List<InskytecProfileSetting>profileSettings, StreamConfig condiction)
        {
    ...
    public class InskytecProfileSetting
    {
        public TvStandard TvStandard;
        public CameraMode Mode;
        public UInt16 ProfileId;
        public List<InskytecCompression> Compressions;
    }

    public class InskytecCompression
    {
        public Compression Compression;
        public List<InskytecResolution> Resolutions;
    }

    public class InskytecResolution
    {
        public List<Resolution> Resolutions;
        public Bitrate[] Bitrates;
        public List<InskytecFrameRate> FrameRates;
    }

    public class InskytecFrameRate
    {
        public List<UInt16> FrameRates;
        public InskytecProfileSetting ProfileSetting;
    }

    public partial class ParseCameraModel
    {
        public static void ParseInskytec(XmlElement node, List<CameraModel> list)
        {
            var cameraModel = new InskytecCameraModel();

            String sameAs = Xml.GetFirstElementValueByTagName(node, "SameAs");
            if (!String.Equals(sameAs, ""))
            {
                CopyInskytecProfile(node, cameraModel, sameAs, list);
                return;
            }
                ...
                foreach (XmlElement profileMode in profileModes)
                {
                    ...
                    if (profiles != null && profiles.Count > 0)
                    {
                        foreach (XmlElement profile in profiles)
                        {
                            ...
                            {
                                cameraModel.ProfileSettings.Add(ParseInskytecProfileNode(tvStandard, cameraMode, profile, Convert.ToUInt16(profileId)));
                            }
                        }
                    }
                }
        ...
        private static InskytecProfileSetting ParseInskytecProfileNode(TvStandard tvStandard, CameraMode mode, XmlElement node, UInt16 profileId)
        {
            if(profileId == 0)
            {
            ...
            var profileSetting = new InskytecProfileSetting
                                     {
                                         TvStandard = tvStandard,
                                         Mode = mode,
                                         ProfileId = profileId,
                                         Compressions = new List<InskytecCompression>()
                                     };
            ...
            foreach (XmlElement compression in compressions)
            {
                ...
                {
                    var newCompression = new InskytecCompression
                    {
                        Compression = codec,
                        Resolutions = new List<InskytecResolution>()
                    };
                    ...
                    if (resolutions != null)
                    {
                        foreach (XmlElement resolution in resolutions)
                        {
                            ...
                            var newResolution = new InskytecResolution
                            {
                                ...
                                FrameRates = new List<InskytecFrameRate>()
                            };
                            ...
                            if (framerates != null)
                            {
                                ...
                                {
                                    var newFrameRate = new InskytecFrameRate
                                    {
                                    ...
                                    if (profiles != null && profiles.Count > 0)
                                    {
                                        foreach (XmlElement profile in profiles)
                                        {
                                            newFrameRate.ProfileSetting = ParseInskytecProfileNode(tvStandard, mode, profile, 0);
                                        }
                                    }
                                }
                            }
        ...
        private static void CopyInskytecProfile(XmlElement node, InskytecCameraModel cameraModel, String sameAs, List<CameraModel> list)
        {
            InskytecCameraModel copyFrom = null;
            foreach (var mode in list)
            {
                if (!String.Equals(mode.Model, sameAs)) continue;
                copyFrom = (InskytecCameraModel)mode;
                break;
            }
            if (copyFrom == null)
                return;
