...
namespace App_POSTransactionServer
{
    public partial class POSManager
    {
        ...
        public Dictionary<UInt16, IPOSConnection> GenericPOSSetting { get; protected set; }
        ...
        public XmlDocument ParseExceptionToXml(POS_Exception posException)
        {
            var xmlDoc = new XmlDocument();
            ...
            var manufacture = posException.Manufacture;
            ...
            xmlRoot.AppendChild(Xml.CreateXmlElementWithText(xmlDoc, "TransactionType", posException.TransactionType));
            xmlRoot.AppendChild(Xml.CreateXmlElementWithText(xmlDoc, "Separator", posException.Separator.Replace("\\r", "\r").Replace("\\n", "\n")));
            xmlRoot.AppendChild(Xml.CreateXmlElementWithText(xmlDoc, "IsCapture", posException.IsCapture ? "true" : "false"));
            xmlRoot.AppendChild(Xml.CreateXmlElementWithText(xmlDoc, "IsSupportPOSId", posException.IsSupportPOSId ? "true" : "false"));
            //-------------------------------------------------------------------------
            ...
            foreach (var exception in posException.Exceptions)
            {
                ...
                if(posException.Manufacture == "Generic" && !posException.IsCapture)
                {
                    var tagend = String.IsNullOrEmpty(exception.TagEnd) ? "" : exception.TagEnd.Replace("/s", " ").Replace("/t", "\t");
                    var value = String.IsNullOrEmpty(exception.Value) ? "" : exception.Value.Replace("/s", " ").Replace("/t", "\t");
                    if(!String.IsNullOrEmpty(posException.Separator))
                    {
                        tagend = tagend.Replace(posException.Separator, Environment.NewLine);
                        value = value.Replace(posException.Separator, Environment.NewLine);
                    }

                    exceptionNode.SetAttribute("tagend", tagend);
                    exceptionNode.InnerText = value;

                }
                else
                {
                    exceptionNode.SetAttribute("tagend", String.IsNullOrEmpty(exception.TagEnd) ? "" : exception.TagEnd);
                    exceptionNode.InnerText = String.IsNullOrEmpty(exception.Value) ? "" : exception.Value;
                }
            ...
            foreach (var segment in posException.Segments)
            {
                ...
                {
                    var tagend = String.IsNullOrEmpty(segment.TagEnd) ? "" : segment.TagEnd.Replace("/s", " ").Replace("/t", "\t");
                    var value = String.IsNullOrEmpty(segment.Value) ? "" : segment.Value.Replace("/s", " ").Replace("/t", "\t");
                    if (!String.IsNullOrEmpty(posException.Separator))
                    {
                        tagend = tagend.Replace(posException.Separator, Environment.NewLine);
                        value = value.Replace(posException.Separator, Environment.NewLine);
                    ...
                    segmentNode.SetAttribute("tagend", tagend);
                    segmentNode.InnerText = value;
                }
                else
                {
                    segmentNode.SetAttribute("tagend", String.IsNullOrEmpty(segment.TagEnd) ? "" : segment.TagEnd);
                    segmentNode.InnerText = String.IsNullOrEmpty(segment.Value) ? "" : segment.Value;
                }
            ...
            foreach (var tag in posException.Tags)
            {
                ...
                if (posException.Manufacture == "Generic")
                {
                    var tagend = String.IsNullOrEmpty(tag.TagEnd) ? "" : tag.TagEnd.Replace("/s", " ").Replace("/t", "\t");
                    var value = String.IsNullOrEmpty(tag.Value) ? "" : tag.Value.Replace("/s", " ").Replace("/t", "\t");
                    if (!String.IsNullOrEmpty(posException.Separator))
                    {
                        tagend = tagend.Replace(posException.Separator, Environment.NewLine);
                        value = value.Replace(posException.Separator, Environment.NewLine);
                    ...
                    tagNode.SetAttribute("tagend", tagend);
                    tagNode.InnerText = value;
                }
                else
                {
                    tagNode.SetAttribute("tagend", String.IsNullOrEmpty(tag.TagEnd) ? "" : tag.TagEnd);
                    tagNode.InnerText = String.IsNullOrEmpty(tag.Value) ? "" : tag.Value;
                }
        ...
        public UInt16 GetNewExceptionId(Boolean reverse)
        {
        ...
        public UInt16 GetNewGenericPOSSettingId()
        {
            UInt16 max = 65535; //65535
            for (UInt16 id = 1; id <= max; id++)
            {
                if (Exceptions.ContainsKey(id)) continue;
                return id;
