...
namespace App_NetworkVideoRecorder
{
	public partial class NetworkVideoRecorder
	{
		...
		{
			Graphics g = e.Graphics;
			g.DrawString(Localization["Application_CPUUsage"].Replace("%1", Nvr.Server.CPUUsage.ToString()),
				ApplicationForms.StatusFont, ApplicationForms.StatusFontColor, 25, 9);
		...
		private void TimePanelPaint(Object sender, PaintEventArgs e)
		{
			Graphics g = e.Graphics;
			var timezone = "(GMT ";
			if(Nvr.Server.TimeZone > 0)
				timezone += "+ ";
			else if(Nvr.Server.TimeZone < 0)
				timezone += "- ";

			//only hours, ex +8  -7
			if ((Nvr.Server.TimeZone % 3600) == 0)
			{
				timezone += Convert.ToUInt16(Math.Abs(Nvr.Server.TimeZone/3600));
			}
			else //containes minis, ex +3.5
			{
				timezone += Math.Round(Math.Abs(Nvr.Server.TimeZone/3600.0), 1, MidpointRounding.AwayFromZero);
			}
			//var text = _nvr.Server.Location + @" " + _nvr.Server.DateTime.ToString("yyyy-MM-dd HH:mm:ss");
			var text = timezone + ") " + Nvr.Server.DateTime.ToString("yyyy/MM/dd HH:mm:ss");
		...
		{
			Graphics g = e.Graphics;
			g.DrawString(Nvr.User.Current.Credential.UserName, ApplicationForms.StatusFont, ApplicationForms.StatusFontColor, 25, 9);

			//check used width, adjust panel width, but just once
			if (!_firstCheckUserWidth) return;
			_firstCheckUserWidth = false;

			var text = Nvr.User.Current.Credential.UserName;
			SizeF fSize = g.MeasureString(text, ApplicationForms.StatusFont);
