...
namespace SetupDivision
{
    public sealed class ItemPanel : Panel
    {
        ...
        {
            if (Width <= 250) return;
            Manager.PaintTitleText(g, Localization["Region_ID"]);

            g.DrawString(Localization["Region_Name"], Manager.Font, Manager.TitleTextColor, 164, 13);

            if (Width <= 410) return;
            g.DrawString(Localization["Region_Citys"], Manager.Font, Manager.TitleTextColor, 350, 13);
        }
        ...
        private void ItemPanelPaint(Object sender, PaintEventArgs e)
        {
            ...
            Manager.PaintStatus(g, Division.ReadyState);
            ...
            if (Width <= 250) return;
            Manager.PaintText(g, Division.Id.ToString());

            g.DrawString(Division.Name, Manager.Font, fontBrush, _nameRectangleF);
