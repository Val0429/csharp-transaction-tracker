...
namespace SetupDevice
{
    public sealed class DevicePanel : Panel
    {
        ...
        private static readonly RectangleF _nameRectangleF = new RectangleF(74, 13, 126, 17);
        private static readonly RectangleF _nameRectangleF2 = new RectangleF(44, 13, 126, 17);
        private static readonly RectangleF _addressRectangleF = new RectangleF(200, 13, 160, 17);
        ...
        private void DevicePanelPaint(object sender, PaintEventArgs e)
        {
            ...
            {
                Manager.PaintText(g,
                    (Parent.Controls.Count - Parent.Controls.IndexOf(this) - 1).ToString().PadLeft(2, '0'));
                g.DrawString(Device.Name, Manager.Font, fontBrush, _nameRectangleF);
            }
            else
            {
                if ((Server.Configure == null) || Server.Configure.DisplayDeviceId)
                {
        ...
        private void PaintInformation(Graphics g, ICamera camera, Brush fontBrush)
        {
            ...
            var address = camera.Profile.NetworkAddress;
            ...
            else
            {
                ...
                else if (camera.Model.Type == "SmartPatrolService")
                    g.DrawString("1", Manager.Font, fontBrush, 750, 13);
                else
                    g.DrawString(camera.Profile.StreamId.ToString(), Manager.Font, fontBrush, 750, 13);
            }
        ...
        private void PaintSchedule(Graphics g, ICamera camera, Brush fontBrush)
        {
            if (Server is IVAS)
            {
                if (camera.EventSchedule != null)
                    g.DrawString(RecordScheduleToLocalizationString(camera.EventSchedule.Description), Manager.Font,
                        fontBrush, 200, 13);
            }
            else if (CMS != null)
            {
            ...
            else
            {
                if (camera.RecordSchedule != null)
                    g.DrawString(RecordScheduleToLocalizationString(camera.RecordSchedule.Description), Manager.Font,
                        fontBrush, 200, 13);
        ...
        private void PaintTitle(Graphics g)
        {
            ...
            switch (DataType)
            {
                ...
                case "SearchResult":
                    ...
                    if (CMS != null)
                    {
                        g.DrawString(Localization["DevicePanel_NetworkAddress"], Manager.Font, Manager.TitleTextColor,
                            200, 13);

                        if (Width <= 420) return;
                        g.DrawString(Localization["DevicePanel_LiveStream"], Manager.Font, Manager.TitleTextColor, 350,
                            13);

                        if (Width <= 540) return;
                        g.DrawString(Localization["DevicePanel_Manufacturer"], Manager.Font, Manager.TitleTextColor, 460,
                            13);
                    ...
                    else
                    {
                        ...
                        if (Width <= 420) return;
                        g.DrawString(Localization["DevicePanel_Compression"], Manager.Font, Manager.TitleTextColor, 350,
                            13);

                        if (Width <= 540) return;
                        g.DrawString(Localization["DevicePanel_Resolution"], Manager.Font, Manager.TitleTextColor, 460,
                            13);
                        ...
                        if (Width <= 855) return;
                        g.DrawString(Localization["DevicePanel_LiveStream"], Manager.Font, Manager.TitleTextColor, 750,
                            13);

                        if (Width <= 960) return;
                        g.DrawString(Localization["DevicePanel_Manufacturer"], Manager.Font, Manager.TitleTextColor, 860,
                            13);
                ...
                case "Schedule":
                    if (Server is IVAS)
                    {
                        g.DrawString(Localization["Control_PeopleCounting"], Manager.Font, Manager.TitleTextColor, 200,
                            13);
                    }
                    else if (CMS != null)
                    {
                        if (Width <= 610) return;
                        g.DrawString(Localization["DevicePanel_EventHandling"], Manager.Font, Manager.TitleTextColor,
                            460, 13);
                    }
                    else
                    {
                        g.DrawString(Localization["DevicePanel_VideoRecording"], Manager.Font, Manager.TitleTextColor,
                            200, 13);
                        ...
                        if (Width <= 710) return;
                        g.DrawString(Localization["DevicePanel_PreEventRecording"], Manager.Font, Manager.TitleTextColor,
                            610, 13);

                        if (Width <= 850) return;
                        g.DrawString(Localization["DevicePanel_PostEventRecording"], Manager.Font,
                            Manager.TitleTextColor, 750, 13);
                    }
                    break;

                case "EventHandle":
                    g.DrawString(Localization["DevicePanel_EventHandling"], Manager.Font, Manager.TitleTextColor, 200,
                        13);
                    break;
            }
        }
        ...
        public string EventHandlerToString(EventHandling eventHandling)
        {
            ...
            foreach (var obj in eventHandling)
            {
        ...
        private string DurationToText(uint duration)
        {
            if (duration < 60)
        ...
        private string SecToStr(uint sec)
        {
            return sec + Localization["Common_Sec"];
        }

        private string MinToStr(uint min)
        {
            var str = "";
            if (min >= 60)
        ...
        private string HrToStr(uint hr)
        {
            var str = hr/3600 + Localization["Common_Hr"];
            if (hr%3600 != 0)
