...
namespace App_CentralManagementSystem
{
	public partial class CentralManagementSystem
	{
		...
		protected override void InitializeHeaderPanel()
		{
			...
			//----------------------------------------
			AppName = Localization["Application_CentralManagementSystem"];
		...
		private void UpdateMenuVisible()
		{
			...
			switch (PageActivated.Name)
			{
				...
				case "Playback":
					FullscreenMenu.Visible = true;
					BandwidthMenu.Visible = true;//playback DO support
					break;
            ...
            if(CMS.Configure.EnableBandwidthControl)
            {
                BandwidthMenu.Visible = false;
            }
            else
            {
                if (!CMS.Configure.CustomStreamSetting.Enable && CMS.Configure.CustomStreamSetting.Bitrate == Bitrate.NA)
                {
		...
		private void SetVideoStreamToOriginal(Object sender, EventArgs e)
		{
			...
			CMS.Configure.CustomStreamSetting.Enable = false;
		...
		private void SetVideoStreamTo1M(Object sender, EventArgs e)
		{
			...
			CMS.Configure.CustomStreamSetting.Enable = true;

			CMS.Configure.CustomStreamSetting.Compression = Compression.H264;
